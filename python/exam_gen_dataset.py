...
def _exec_generate_dataset(args):
    ...
    for dataset in dataset_list:
        ...
        print('------------------\nProcessing ' +
                data_root, ', type: ' + motion_type)
        # drop the head and tail
        pose_data = np.genfromtxt(
            data_root+'/pose.txt')[args.skip_front:-args.skip_end, :]
        ...
        output_samplerate = output_timestamp.shape[0] * nano_to_sec / (
        ...
        orientation_data[:, [1, 2, 3, 4]
                            ] = orientation_data[:, [4, 1, 2, 3]]
